...
class plgSystemPlg_Sj_Content_Listing_Ajax extends JPlugin {
	...
	function escape($var){
	 ...
	 function getFileDefault($file_name = 'item'){
		if($this->_view == 'view_category' ){
	 ...
	 function getViewHtml($lead_items, $intro_items, $link_items,$cont_params){
		ob_start(); 
		$leadingcount = 0;
		if (!empty($lead_items)) : ?>
		<div class="<?php echo $this->cls_items_leading1 ?> items-leading">
			<?php foreach ($lead_items as &$item) : ?>
			<div class="<?php echo $this->cls_items_leading2 ?> item leading-<?php echo $leadingcount; ?><?php echo $item->state == 0 ? ' system-unpublished' : null; ?>">
				<?php
					ob_start();
					$this->item = $item;
		...
		if ($rowcount == 1) : ?>
		<div class="<?php echo $this->cls_items_row1 ?> items-row cols-<?php echo (int) $this->columns;?> <?php echo 'row-'.$row; ?> row ">
		<?php endif; ?>
			<div class="<?php echo $this->cls_items_row2 ?> item col-sm-<?php echo round((12 / $this->columns)); ?> <?php echo ($this->columns > 1) ? 'grid' : ''?> column-<?php echo $rowcount; ?>" itemprop="blogPost" itemscope itemtype="http://schema.org/BlogPosting">
				<?php
				ob_start();
				$this->item = $item;
				$this->params = $cont_params;
		...
		<?php endif;
		if (!empty($link_items)) :  ?>
		<div class="<?php echo $this->cls_items_more; ?> items-more">
			<?php
			ob_start();
	 ...
	 function addjQuery(){
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				;(function(element){
					var $element 	= $(element),
						$load_more = $('.resplc-loadmore'),
						$btn_load 	= $('.resplc-loadmore-btn');
